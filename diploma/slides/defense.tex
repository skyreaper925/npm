...
\begin{frame}{Метод: проекционный метод и сетки Коробова}
  ...
  \begin{itemize}
    ...
    Лог-интерполяция $\Rightarrow$ положительность и $H$-теорема.
...
\begin{frame}{Верификация 1. Квази-шок (одногазовая релаксация)}
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
      \includegraphics[width=\textwidth]{sl_t0_T}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Верификация 1. $H$-теорема}
  \centering
  \includegraphics[width=0.62\textwidth]{sl_t0_H}\\
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}\small
        ...
        \item Равновесие $T=\dfrac{\sum_i n_i T_i}{\sum_i n_i}=1{,}5$.
        \item Концентрации сохраняются; суммарная энтропия растёт.
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \includegraphics[width=\textwidth]{sl_t1_T}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Верификация 2. Рост энтропии смеси}
  \centering
  \includegraphics[width=0.6\textwidth]{sl_t1_S}\\
...
\begin{frame}{Перенос: результаты (установившаяся УВ, $L=80$, $N_x=400$)}
  \centering
  \includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{sl_t2_profiles}\\
  {\scriptsize Профили $n,\,u_x,\,T,\,T_{xx}$: сжатие $n\to3$, нагрев $T\to3{,}6$, «перескок» $T_{xx}$ на фронте.}
\end{frame}
...
\begin{frame}{Перенос: фронт волны и проверка Гюгонио}
  \begin{columns}
    \begin{column}{0.52\textwidth}
      \centering\includegraphics[width=\textwidth]{sl_t2_front}\\
      {\small Фронт $x_c(t)$ линеен $\Rightarrow V$; \quad $n_{\max}\to3$.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering\includegraphics[width=\textwidth]{sl_t2_fx}\\
